...
void countOpenAndCloseEdges(const int& ix, const int& iy, queue<long long int>& stack, BIT& tree){
  ...
  int ip = iu < iv ? iu : iv;
  if( tmp->isOpen() ){
    //printf(" true:iu:%d iv:%d, pos of ix:%d, pos of iy:%d, ix:%d, iy:%d\n", iu, iv, ix, v[iy], pos[ix], pos[iy]);
    assert (ip != ix);
    tree.add(ip, -1);
    stack.push(i);
  }
  else{
    //printf("false:iu:%d iv:%d, pos of ix:%d, pos of iy:%d, ix:%d, iy:%d\n", iu, iv, ix, v[iy], pos[ix], pos[iy]);
    assert (ip == ix);
    tree.add(ip, 1);
  }
  tmp->reverseOpen();
...
void countSingleEdge(long long int& count, const BIT& tree, const queue<long long int>& stack){
  ...
  if(iv - iu > 1) 
    count += (long long int)tree.sum(iu + 1, iv - 1);
  else if (iu - iv > 1)
    count += (long long int)tree.sum(iv + 1, iu - 1);
}
